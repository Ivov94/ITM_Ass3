...
public class AudioThumbGenerator {
	...
	protected File processAudio(File input, File output) throws IOException,
		...
		// load the input audio file
				...
				long dataThumb = thumbNailLength * (long)decodedFormat.getFrameRate();
				
				if (AudioSystem.isConversionSupported(decodedFormat, baseFormat) || decodedFormat.equals(baseFormat)) {
					decodedAudioInput = AudioSystem.getAudioInputStream(decodedFormat, audioInput);
				    
				    if (baseFormat.getEncoding().equals(new AudioFormat.Encoding("VORBISENC"))) {
						
			            byte [] buffer = new byte[4096];
			            int n;
			            
			            FileOutputStream fos = new FileOutputStream(outputFile + ".pcm");    
			            while(-1 != (n = decodedAudioInput.read(buffer))) {
			                fos.write(buffer, 0, n); 
			            }    
			            fos.close();
			            
			            // cut the audio data in the stream to a given length
			            AudioInputStream temporary = new AudioInputStream(new FileInputStream(outputFile + ".pcm"), decodedFormat, dataThumb);
			            // save the acoustic thumbnail as WAV file
			            AudioSystem.write(temporary, AudioFileFormat.Type.WAVE, new FileOutputStream(outputFile));
			            
			            temporary.close();
			            
			            File fileToDelete = new File(outputFile + ".pcm");
			            fileToDelete.delete();
			        }
				    else{
				    	// cut the audio data in the stream to a given length
					    ...
					    // save the acoustic thumbnail as WAV file
				    	AudioSystem.write(Thumbnail, new AudioFileFormat.Type("WAVE", "wav"), outputFile);
				    }
				}
				else {
