...
public class VideoThumbnailGenerator {
	...
	protected File processVideo(File input, File output, boolean overwrite, int timespan) throws Exception {
		...
		for(int i=0;i<list.size();++i){
			System.out.println(output.getAbsolutePath()+ File.separator + input.getName()+i);
			ImageIO.write(list.get(i), "jpg", new File(output.getAbsolutePath()+ File.separator + input.getName()+i+".jpg"));
		}
